...
{
    public record RussianLegalEntityDetails : CounterpartyDetails
    {
        ...
        private RussianLegalEntityDetails(NonEmptyString taxNumber, NonEmptyString kpp)
        {
            TaxNumber = taxNumber;
            KPP = kpp;
        ...
        public static Result<RussianLegalEntityDetails> Create(string taxNumber, string kpp)
        {
            ...
            var taxNumberValid = Result.FailIf(!AccountValidator.IsThisCompanyINN(taxNumberNES.Value), "INN is invalid.");
            ...
            return Result.Ok(new RussianLegalEntityDetails(taxNumberNES.Value, kppNumberNES.Value));
        }
